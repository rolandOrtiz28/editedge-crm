...
const Contacts = ({ setGroups }) => {
  ...
  const handleDeleteContact = (id) => {
    axios
      .delete(`${API_BASE_URL}/api/contacts/${id}`)
      .then(() => {
  ...
  const handleUpdateContact = () => {
    setLoading(true);
    axios
      .put(`${API_BASE_URL}/api/contacts/${editContact._id}`, editContact)
      .then((res) => {
        setContacts(
  ...
  const handleDeleteAllContacts = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: "All contacts have been removed successfully.",
      });
  ...
  const handleCsvUploadWithChoice = async (createGroup) => {
    ...
    try {
      const response = await axios.post(
        `${API_BASE_URL}/api/contacts/upload-csv`,
        formData,
        {
      ...
      toast({
        ...
        description: createGroup
          ? "Contacts imported & grouped successfully!"
  ...
  const filteredContacts = selectedGroup === "all"
    ...
    : groups
        ...
        ?.members.map((member) =>
          contacts.find((contact) => contact._id === member.memberId)
        )
        .filter(
          (contact) =>
            contact &&
  ...
  return (
    <div>
      <PageHeader
        title="Contacts"
        subtitle="Manage your contacts and stay connected with your network."
        icon={Contact}
      />
          ...
          <div className="flex gap-2">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end" className="w-48">
                ...
                >
                  <Trash className="mr-2 h-4 w-4" /> Delete All Contacts
                </DropdownMenuItem>
              </DropdownMenuContent>
            </DropdownMenu>
      ...
      <Dialog open={isDeleteAllDialogOpen} onOpenChange={setIsDeleteAllDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Are you sure you want to delete all contacts? This action cannot be
              undone.
            </DialogDescription>
          </DialogHeader>
