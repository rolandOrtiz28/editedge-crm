...
const Messages = () => {
    ...
    const fetchConversations = () => {
        const endpoint = activeTab === "Messenger" 
    ...
    const loadMessages = (conversationId) => {
        ...
        axios.get(endpoint)
            .then(response => {
                ...
                const otherUser = conversations.find(conv => conv.id === conversationId)?.participants?.data?.[0]?.name || "Unknown";
    ...
    const sendMessage = () => {
        ...
        const endpoint = activeTab === "Messenger"
            ...
            : `${API_BASE_URL}/api/instagram/send`;
        ...
        .catch(error => console.error(`Error sending ${activeTab} message:`, error));
        ...
        <div className="flex h-screen bg-gray-100">
            <div className="w-1/3 border-r bg-white p-4 overflow-y-auto">
                ...
                <div className="flex mb-4">
                    <button 
                        className={`w-1/2 p-2 text-center ${activeTab === "Messenger" ? "border-b-2 border-brand-black font-bold" : "text-gray-500"}`} 
                        onClick={() => setActiveTab("Messenger")}
                    >
                        Messenger
                    </button>
                    <button 
                        className={`w-1/2 p-2 text-center ${activeTab === "Instagram" ? "border-b-2 border-brand-black font-bold" : "text-gray-500"}`} 
                    ...
                    <>
                        <div id="chat-container" className="flex-1 overflow-y-auto p-4 bg-gray-50 space-y-3">
                            {messages.map((msg, index) => (
                                <div key={index} className={`flex flex-col items-start mb-3 ${msg.from.name === "You" ? "items-end" : "items-start"}`}>
                                    <span className="text-xs text-gray-500 mb-1">{msg.from.name}</span>
                                    <div className={`rounded-lg p-3 max-w-xs ${msg.from.name === "You" ? "bg-brand-black text-white" : "bg-gray-200 text-black"}`}>
                                        <p className="text-sm">{msg.message}</p>
                                        <span className="text-xs block mt-1 opacity-70">{new Date(msg.created_time).toLocaleTimeString()}</span>
                                    </div>
                                </div>
                            ))}
                        </div>
